...
try {
    $stmt = $pdo->prepare("SELECT idInstructor, CONCAT(nombre, ' ', apellidoPaterno, ' ', apellidoMaterno) AS nombreCompleto, edad FROM instructor");
    $stmt->execute();
...
<body>
    ...
    <div class="container-fluid mt-5">
        <div class="container">
            ...
            <div class="table-responsive">
                <!-- Botón para abrir el modal de nuevo instructor -->
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-secondary btnEditarInstructor"
                                        ...
                                        data-nombre="<?= htmlspecialchars($i['nombreCompleto']) ?>"
                                        data-edad="<?= htmlspecialchars($i['edad']) ?>"
                                        data-bs-toggle="modal" data-bs-target="#modalEditarInstructor">Editar</button>

                                    <form action="/GestiFit/public/php/eliminarInstructor.php" method="POST" class="d-inline" onsubmit="return confirm('¿Seguro que quieres eliminar este instructor?');">
                                        <input type="hidden" name="idInstructor" value="<?= $i['idInstructor'] ?>">
    ...
    <div class="modal fade" id="agregarInstructorModal" tabindex="-1" aria-labelledby="agregarInstructorModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="/GestiFit/public/php/agregarInstructor.php" method="POST">
    ...
    <div class="modal fade" id="modalEditarInstructor" tabindex="-1" aria-labelledby="modalEditarInstructorLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form action="/GestiFit/public/php/editarInstructor.php" method="POST" class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Editar Instructor</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body row">
                    <input type="hidden" name="idInstructor" id="edit_idInstructor">
                    <div class="mb-3 col-md-6">
                        <label for="nombre" class="form-label">Nombre</label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.btnEditarInstructor').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Suponemos que nombreCompleto tiene tres partes
                    const [nombre, apellidoPaterno, ...rest] = nombreCompleto.split(' ');
                    const apellidoMaterno = rest.join(' ');
