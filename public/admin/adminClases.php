...
try {
    // Consulta principal para obtener clases con nombre del instructor
    $stmt = $pdo->prepare("SELECT c.idClase, c.nombreClase, c.horario, c.cuposDisponibles, c.cuposOcupados,
                              c.idInstructor,
                              CONCAT(i.nombre, ' ', i.apellidoPaterno, ' ', i.apellidoMaterno) AS nombreInstructor
                       FROM clase c
    ...
    $diasPorClase = [];
    $stmtDias = $pdo->prepare("SELECT idClase, dia FROM clasedias");
    $stmtDias->execute();
    foreach ($stmtDias->fetchAll(PDO::FETCH_ASSOC) as $fila) {
        $diasPorClase[$fila['idClase']][] = $fila['dia'];
    }
} catch (PDOException $e) {
    error_log("Error al obtener las clases: " . $e->getMessage());
    $clases = [];
    $diasPorClase = [];
}

try {
    $stmtInstructores = $pdo->query("SELECT idInstructor, nombre, apellidoPaterno, apellidoMaterno FROM instructor");
