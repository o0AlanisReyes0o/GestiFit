...
// Obtener lista de clientes con sus membresías
$queryClientes = "SELECT u.*, m.nombre AS membresia, um.fechaInicio, um.fechaFin, um.idMembresia 
                 FROM usuario u 
...
// Obtener lista de membresías para los selects
$queryMembresias = "SELECT * FROM membresia";
$resultMembresias = mysqli_query($conexion, $queryMembresias);
$membresias = [];
if ($resultMembresias) {
    while ($row = mysqli_fetch_assoc($resultMembresias)) {
        $membresias[] = $row;
    }
}
...
if (!empty($nombreFiltro) || !empty($membresiaFiltro)) {
    $queryFiltrada = "SELECT u.*, m.nombre AS membresia, um.fechaInicio, um.fechaFin 
                     FROM usuario u 
                     LEFT JOIN usuariomembresia um ON u.idUsuario = um.idUsuario 
                     LEFT JOIN membresia m ON um.idMembresia = m.idMembresia 
                     WHERE u.tipo = 'cliente'";
    ...
    if (!empty($nombreFiltro)) {
        $queryFiltrada .= " AND (u.nombre LIKE '%$nombreFiltro%' OR u.apellidoPaterno LIKE '%$nombreFiltro%' OR u.apellidoMaterno LIKE '%$nombreFiltro%')";
    }
    
    if (!empty($membresiaFiltro)) {
        $queryFiltrada .= " AND m.nombre LIKE '%$membresiaFiltro%'";
    }
    
    $queryFiltrada .= " ORDER BY u.nombre";
    
    $resultFiltrado = mysqli_query($conexion, $queryFiltrada);
    $usuarios = [];
    if ($resultFiltrado) {
        while ($row = mysqli_fetch_assoc($resultFiltrado)) {
            $usuarios[] = $row;
        }
    }
}
...
<body>
            ...
            <?php if (!empty($usuarios)): ?>
              <?php foreach ($usuarios as $u): ?>
                <tr>
                  ...
                  <td><?= htmlspecialchars($u['fechaInicio'] ?? 'N/A') ?></td>
                  <td><?= htmlspecialchars($u['fechaFin'] ?? 'N/A') ?></td>
                  <td>
                    <button
                      type="button"
                    ...
                    <form method="post" action="/GestiFit/src/php/eliminarUsuario.php" style="display:inline;" onsubmit="return confirm('¿Seguro que quieres eliminar este usuario?');">
  ...
  <div class="modal fade" id="editarUsuarioModal" tabindex="-1" aria-labelledby="editarUsuarioModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <form id="formEditarUsuario" method="POST" action="/GestiFit/src/php/editarUsuario.php">
  ...
  <div class="modal fade" id="modalAgregarCliente" tabindex="-1" aria-labelledby="modalAgregarClienteLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <form id="formAgregarCliente" method="POST" action="/GestiFit/src/php/agregarCliente.php">
        <div class="modal-content">
