...
try {
    ...
    $stmtInstructores = $pdo->query("SELECT i.idInstructor, CONCAT(i.nombre, ' ', i.apellidoPaterno) AS nombre, COUNT(c.idClase) AS totalClases FROM instructor i LEFT JOIN clase c ON i.idInstructor = c.idInstructor GROUP BY i.idInstructor");
    ...
    $stmtClases = $pdo->query("SELECT c.nombreClase, COUNT(uc.idUsuario) AS inscritos FROM clase c LEFT JOIN usuarioclase uc ON c.idClase = uc.idClase GROUP BY c.idClase");
    $clasesData = $stmtClases->fetchAll(PDO::FETCH_ASSOC);
    $clases = ["labels" => [], "data" => []];
    foreach ($clasesData as $clase) {
        $clases['labels'][] = $clase['nombreClase'];
        $clases['data'][] = (int)$clase['inscritos'];
    }

    // Membresías (distribución por cantidad de usuarios)
    $stmtMembresias = $pdo->query("SELECT m.nombre, COUNT(um.idUsuario) AS total FROM usuariomembresia um JOIN membresia m ON um.idMembresia = m.idMembresia GROUP BY m.idMembresia");
